...
class LidarRecorder:
  ...
  def _apply_perturbation(
    ...
    if p.global_position_fixed and self._perturbation_fixed_location is not None:
      ...
      base_loc = carla.Location(
        x=ego_transform.location.x + p.spawn_distance_from_ego * np.cos(angle_rad),
        y=ego_transform.location.y + p.spawn_distance_from_ego * np.sin(angle_rad),
    ...
    if self._perturbation_actor is None or not self._perturbation_actor.is_alive:
      ...
      try:
        actor = self.world.try_spawn_actor(self.blueprint_library.find(p.blueprint), transform)
        if not actor:
  ...
  def initialize_scene(
    ...
    name_for_map = Path(source_instance).name if source_instance else instance_path.name
    town_name = next((p for p in name_for_map.split('_') if p.startswith('Town')), None)
    if not town_name:
  ...
  def process_frame(
    ...
    for bb in data.get('bounding_boxes', []):
      ...
      if actor_id not in self.spawned_actors and actor_id not in self.failed_spawn_ids:
        try:
          bp = self._resolve_blueprint(actor_class, bb.get('type_id'))
          actor = self.world.try_spawn_actor(bp, transform) or self.world.try_spawn_actor(
            bp, carla.Transform(transform.location + carla.Location(z=2.0), transform.rotation)
          )
          if actor:
            actor.set_simulate_physics(False)
